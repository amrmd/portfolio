...
describe('AmaPageView', () => {
  ...
  it('lists all seven topics in each locale', () => {
    ...
    expect(zh.container.textContent).toContain('Homelab')
    expect(zh.container.textContent).toContain('PM、财务和日常运营与自动化')
    expect(zh.container.textContent).toContain('PV 不是一人公司')
    expect(zh.container.textContent).toContain('公司文化里少不了的一部分')
    expect(zh.container.textContent).not.toContain('西雅图')
    expect(zh.container.textContent).not.toContain('Niantic')
    ...
    expect(en.container.textContent).toContain('self-hosted Homelab')
    expect(en.container.textContent).toContain('I help startups')
    expect(en.container.textContent).toContain('day-to-day operations and automations')
    expect(en.container.textContent).not.toContain('game studios in Seattle')
    expect(en.container.textContent).not.toContain('Niantic')
    expect(en.container.textContent).toContain('PV isn’t a one-person company')
  ...
  it('states the 24 hour cancellation policy and drops the testimonials section', () => {
    const zh = render(<AmaPageView locale="zh" />)
